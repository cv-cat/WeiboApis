...
function getFileCS(t) {
    ...
    a = a >>> 8 ^ i[255 & (a ^ t.charCodeAt(o))];
    return (-1 ^ a) >>> 0
}

function getFileBinaryString(filePath) {
    let data = fs.readFileSync(filePath, 'binary');
    data = data.toString();
    return data;
}

function generateParams(filePath) {
    const binaryString = getFileBinaryString(filePath);
    const cs = getFileCS(binaryString);
...
const filePath = 'D:/Desktop/Snipaste_2025-01-14_14-53-27.jpg';
const params = generateParams(filePath);
console.log(params);